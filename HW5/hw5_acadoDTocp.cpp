...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) 
 { 
    ...
    acadodata_f1 << next(x1) == (1.00000000000000000000e+00-u)*x1;
    acadodata_f1 << next(x2) == sqrt((x1*x1+x2*x2))*u;

    Grid grid_acadodata_v1(acadodata_v1);
    OCP ocp1(grid_acadodata_v1);
    ocp1.minimizeLagrangeTerm((x1*x1+x2*x2));
    ocp1.subjectTo(acadodata_f1);
    ocp1.subjectTo(AT_START, x1 == 1.00000000000000000000e+00);
    ocp1.subjectTo(AT_START, x2 == 1.00000000000000000000e+00);
    ocp1.subjectTo(0.00000000000000000000e+00 <= u <= 1.00000000000000000000e+00);
    ocp1.subjectTo(AT_END, x1 == 0.00000000000000000000e+00);
