...
function hw5_CTdoubleIntMPCNoTerm
...
% f1 = figure(1); hold on
%plot initial point
fig2 = figure('Name','Question 3','units','normalized', ...
    'outerposition',[0 0 1 1], 'NumberTitle','off');
ax9 = subplot(4,2,[7 8]); grid on; hold on;
ax8 = subplot(4,2,[2 4 6]); grid on; hold on;
ax7 = subplot(4,2,[1 3 5]); grid on; hold on;
...
%     beq=xcloop;
    ...
    [y_OL, V, exitflag, output]=fmincon(@(y) costfunction( N, y, x_eq, u_eq, Q, R,n,m,delta),...
    ...
    xlabel('x(1)')
    ylabel('x(2)')
    strTitle = ['Running MPC with Terminal Constraints ... #',num2str(ii),' /',num2str(mpciterations)];
    title(strTitle);
    axis equal
    drawnow
  
end
ax9 = subplot(4,2,[7 8]); grid on; hold on; yyaxis left
stairs(t,u);
...
function cost = costfunction(N, y, x_eq, u_eq, Q, R,n,m,delta)
