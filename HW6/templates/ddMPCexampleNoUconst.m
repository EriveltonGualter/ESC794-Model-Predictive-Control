...
%Terminal equilibrium constraints, no input constraints
...
function ddMPCexampleNoUconst

    clear;clc;close all
    %Define decoupled plant
    Ad=[2 2 0;-1 1 0;0 0 4];
    Bd=[1 0;0 0;0 1];
    ...
    [Qbar1,Rbar1,xeq1_hat,ueq1_hat]=buildQRbarEq(xeq1,ueq1,Q1,R1,ny);
    ...
    [Qbar2,Rbar2,xeq2_hat,ueq2_hat]=buildQRbarEq(xeq2,ueq2,Q2,R2,ny);
    ...
    for k=1:simhor
       ...
       while ~converged 
           ...
           t1=tic;
           %Use analytical solution to Y1=quadprog(S1,f1);
           Y1=-S1\f1';
           dt1=toc(t1);
           T1=T1+dt1;

           t2=tic;
           %Use analytical solution to Y2=quadprog(S2,f2);
           Y2=-S2\f2';
           dt2=toc(t2);
           T2=T2+dt2;
           ...
           %interleave Y1 and Y2 solutions
           %HARD-CODED FOR GIVEN DIMENSIONS AND
           %HORIZON 
    ...
    subplot(3,1,1)
    title('States: Centralized vs. Parallel MPC: No Input Constraints','Fontsize',14)
    ...
    legend('Centralized','Distrib')
    subplot(2,1,1)
    title('Control Inputs: Centralized vs. Parallel MPC: No Input Constraints','Fontsize',14)
    ...
    stairs(Ttotal2,'k--');
    stairs(T,'k')
    title('Computation Time: Centralized vs. Parallel MPC: No Input Constraints','Fontsize',14)
